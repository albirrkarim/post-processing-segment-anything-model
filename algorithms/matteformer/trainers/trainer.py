...
import torch.nn.utils_matteformer as nn_utils_matteformer
import torch.backends.cudnn as cudnn
from torch.nn import SyncBatchNorm
import torch.optim.lr_scheduler as lr_scheduler
from torch.nn.parallel import DistributedDataParallel

import utils_matteformer
from utils_matteformer import CONFIG
import networks
...
class Trainer(object):
    ...
    def __init__(self,
        ...
        utils_matteformer.print_network(self.G, CONFIG.version)
    ...
    def train(self):
        ...
        for step in range(start, self.train_config.total_step + 1):
            ...
            if step < self.train_config.warmup_step and self.train_config.resume_checkpoint is None:
                cur_G_lr = utils_matteformer.warmup_lr(self.train_config.G_lr, step + 1, self.train_config.warmup_step)
                utils_matteformer.update_lr(cur_G_lr, self.G_optimizer)
            ...
            weight_os8 = utils_matteformer.get_unknown_tensor(trimap)
            weight_os8[...] = 1

            if step < self.train_config.warmup_step:
                weight_os4 = utils_matteformer.get_unknown_tensor(trimap)
                weight_os1 = utils_matteformer.get_unknown_tensor(trimap)
                ...
                if random.randint(0, 1) == 0:
                    ...
                    weight_os4 = utils_matteformer.get_unknown_tensor_from_pred(alpha_pred_os8, rand_width=CONFIG.model.self_refine_width1, train_mode=True)
                    alpha_pred_os4[weight_os4 == 0] = alpha_pred_os8[weight_os4 == 0]
                    weight_os1 = utils_matteformer.get_unknown_tensor_from_pred(alpha_pred_os4, rand_width=CONFIG.model.self_refine_width2, train_mode=True)
            ...
            if self.train_config.clip_grad:
                if moving_max_grad == 0:
                    moving_max_grad = nn_utils_matteformer.clip_grad_norm_(self.G.parameters(), 1e+6)
                    max_grad = moving_max_grad
                else:
                    max_grad = nn_utils_matteformer.clip_grad_norm_(self.G.parameters(), 2 * moving_max_grad)
    ...
    def test(self, step, start):
        ...
        with torch.no_grad():
            for idx, image_dict in enumerate(self.test_dataloader):
                ...
                weight_os4 = utils_matteformer.get_unknown_tensor_from_pred(alpha_pred, rand_width=CONFIG.model.self_refine_width1, train_mode=False)
                alpha_pred[weight_os4 > 0] = alpha_pred_os4[weight_os4 > 0]
                weight_os1 = utils_matteformer.get_unknown_tensor_from_pred(alpha_pred, rand_width=CONFIG.model.self_refine_width2, train_mode=False)
                ...
                weight = utils_matteformer.get_unknown_tensor(trimap)  # get unknown region (trimap)
        ...
        if CONFIG.dist:
            self.test_loss_dict = utils_matteformer.reduce_tensor_dict(self.test_loss_dict, mode='mean')
    ...
    def write_log(self, loss, step, image, trimap, cur_G_lr):
        ...
        # reduce losses from GPUs
        if CONFIG.dist:
            self.loss_dict = utils_matteformer.reduce_tensor_dict(self.loss_dict, mode='mean')
            loss = utils_matteformer.reduce_tensor(loss)

...
def single_inference(model, image_dict, device="cuda"):
    ...
    with torch.no_grad(): 
        ...
        weight_os4 = utils_matteformer.get_unknown_tensor_from_pred(alpha_pred, rand_width=CONFIG.model.self_refine_width1, train_mode=False,device=device)
        alpha_pred[weight_os4>0] = alpha_pred_os4[weight_os4>0]
        weight_os1 = utils_matteformer.get_unknown_tensor_from_pred(alpha_pred, rand_width=CONFIG.model.self_refine_width2, train_mode=False,device=device)
...
if __name__ == '__main__':
    ...
    utils_matteformer.make_dir(os.path.join(args.output, 'pred_alpha'))
    ...
    model.load_state_dict(utils_matteformer.remove_prefix_state_dict(checkpoint['state_dict']), strict=True)

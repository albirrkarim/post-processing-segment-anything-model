...
import toml
import torch
from torch.utils_matteformer.data import DataLoader
...
def copy_script(root_path=None):
    ...
    shutil.copytree('./utils_matteformer', os.path.join(root_path, 'utils_matteformer'), ignore=shutil.ignore_patterns('__pycache__'))
...
def main():
    ...
    if CONFIG.phase.lower() == "train":
        ...
        if CONFIG.local_rank == 0:
            utils_matteformer.make_dir(CONFIG.log.logging_path)
            utils_matteformer.make_dir(CONFIG.log.checkpoint_path)

        """=== Set logger ==="""
        logger = utils_matteformer.get_logger(CONFIG.log.logging_path, logging_level=CONFIG.log.logging_level)
        ...
        if CONFIG.dist:
            train_sampler = torch.utils_matteformer.data.distributed.DistributedSampler(train_dataset)
            test_sampler = torch.utils_matteformer.data.distributed.DistributedSampler(test_dataset)
        else:
            train_sampler = None
            test_sampler = None
...
if __name__ == '__main__':
    ...
    with open(args.config) as f:
        utils_matteformer.load_config(toml.load(f))
